...
void init_simlib() {
  /* Initialize simlib.c.  List LIST_EVENT is reserved for event list, ordered
     by event time.  init_simlib must be called from main by user. */
    ...
    if ((option == INCREASING) || (option == DECREASING)) {
      ...
      if (option == INCREASING) {
        ...
        while (postest) {
          ...
          if (postest) postest = (transfer[item] >= (*row).value[item]);
        }
      }
...
double sampst(double value, int variable) {
  /* Initialize, update, or report statistics on discrete-time processes:
     sum/average, max (default -1E30), min (default 1E30), number of
     observations for sampst variable "variable", where "variable": = 0
     initializes accumulators > 0 updates sum, count, min, and max accumulators
     with new observation < 0 reports stats on variable "variable" and returns
     them in transfer: [1] = average of observations [2] = number of
     observations [3] = maximum of observations [4] = minimum of observations */
...
double expon(double mean, int stream) /* Exponential variate generation
                                         function. */
{
  return -mean * log(lcgrand(stream));
}

int random_integer(double prob_distrib[], int stream) /* Discrete-variate
                                                         generation function. */
{
  int i;
  double u;
...
double uniform(double a, double b, int stream) /* Uniform variate generation
                                                  function. */
{
  return a + lcgrand(stream) * (b - a);
}

double erlang(int m, double mean, int stream) /* Erlang variate generation
                                                 function. */
{
  int i;
  ...
  sum = 0.0;
  for (i = 1; i <= m; ++i) sum += expon(mean_exponential, stream);
  return sum;
}
